...
# Multi-Service Cloud-Native CI/CD Pipeline Framework
A production-grade, highly optimized GitHub Actions CI/CD framework automating container workflows and secure cloud deployments for a microservices-based retail store application.
The Microservice Container Pipeline (ECS Fargate)
**Goal**: Automatically build a Docker image from a Dockerfile, push it to Amazon ECR (Elastic Container Registry), and deploy it to an ECS Fargate cluster.

## 🏗️ Architecture & Deployment Flow
This repository showcases an automated, decoupled monorepo pipeline strategy designed to deploy individual microservices (`catalog`, `cart`, `checkout`, `orders`) to **AWS ECS Fargate** with zero downtime.

```text
[Code Change Detected]
       │
       ▼
 1. Path Filtering Verification (Isolates changed service only)
       │
       ▼
 2. AWS Authentication via OIDC (Short-lived dynamic token generation)
       │
       ▼
 3. Context-Aware Docker Build (Navigates to specific service directory)
       │
       ▼
 4. Secure ECR Push (Tags image uniquely using short Git Commit SHA)
       │
       ▼
 5. ECS Task Definition Update & Graceful Rolling Deployment

 ### 🔒 Enterprise Security via OIDC
Instead of storing dangerous, long-lived `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` credentials inside GitHub Secrets, this framework leverages **OpenID Connect (OIDC)**. The GitHub runner dynamically authenticates with AWS IAM via temporary tokens utilizing a precise cryptographic permissions block:
```yaml
permissions:
  id-token: write
  contents: read
